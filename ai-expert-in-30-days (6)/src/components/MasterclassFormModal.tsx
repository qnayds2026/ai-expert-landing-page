...
export const MasterclassFormModal: React.FC<MasterclassFormModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Meta Pixel: Lead & InitiateCheckout Tracking
    try {
      if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
        window.fbq('track', 'Lead');
        window.fbq('track', 'InitiateCheckout', {
          value: PAYMENT_AMOUNT_INR,
          currency: 'INR',
          content_name: 'AI Masterclass 1-Hour Live Pass'
        });
      }
    } catch {
      // ignore
      ...
      try {
        const options = {
          ...
          handler: function (_response: any) {
            // Meta Pixel: Purchase Event Tracking on verified payment
            try {
              if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-slate-900 leading-tight">
                {currentStep === 'form' && `Join 1-Hour Live Masterclass (₹${PAYMENT_AMOUNT_INR})`}
                {currentStep === 'payment' && `Step 2: Payment (₹${PAYMENT_AMOUNT_INR})`}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-blue-50/80 border border-blue-200 text-slate-800 text-xs sm:text-sm space-y-1">
                <div className="flex items-center gap-2 font-bold text-blue-900">
                  ...
                  <span>1-Hour Live Masterclass (₹{PAYMENT_AMOUNT_INR}) Registration</span>
                </div>
                <p className="text-slate-600 text-xs leading-relaxed">
                  Fill in your details below to register for the 1-Hour Live AI Masterclass. After submitting, you will proceed to complete the ₹{PAYMENT_AMOUNT_INR} session fee and join the official <strong className="text-blue-700">WhatsApp Cohort Group</strong>.
                </p>
              </div>
              ...
              <div className="pt-4 space-y-3 border-t border-slate-100">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <span>Proceed to Payment (₹{PAYMENT_AMOUNT_INR})</span>
            ...
            <div className="py-2 space-y-5 animate-in zoom-in-95 duration-200">
              ...
              <div className="text-center space-y-1.5">
                ...
                <h3 className="text-xl sm:text-2xl font-black text-slate-900">
                  Pay ₹{PAYMENT_AMOUNT_INR} to Secure Your Seat
                </h3>
              ...
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 text-white p-4 sm:p-5 rounded-2xl shadow-xl relative overflow-hidden">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right bg-white/10 px-3.5 py-2.5 rounded-xl border border-white/15 min-w-[90px] shrink-0">
                    <span className="text-[9px] text-slate-400 block line-through">Regular ₹{ORIGINAL_PRICE_INR}</span>
              ...
              <div className="space-y-2.5 pt-1">
                ...
                >
                  {isPaymentVerifying ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span>Verifying ₹{PAYMENT_AMOUNT_INR} Payment & Unlocking WhatsApp...</span>
                    </>
                  ) : (
                    <>
                      <CreditCard className="w-5 h-5" />
                      <span>{paymentInitiated ? `Re-open Razorpay Gateway (₹${PAYMENT_AMOUNT_INR})` : `Pay ₹${PAYMENT_AMOUNT_INR} via Razorpay / UPI`}</span>
                      <ArrowRight className="w-5 h-5" />
                    </>
                  )}
                </button>
