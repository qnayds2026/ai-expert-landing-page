...
export const MasterclassFormModal: React.FC<MasterclassFormModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const submissionData = {
      ...
      source: 'AI Masterclass 1-Hour Live Pass (₹1)',
  ...
  const handleProcessPayment = (method: 'card' | 'upi' | 'razorpay') => {
    ...
    if (typeof window !== 'undefined' && window.Razorpay && RAZORPAY_KEY_ID && !RAZORPAY_KEY_ID.includes('demo123456789')) {
      try {
        const options = {
          key: RAZORPAY_KEY_ID,
          amount: PAYMENT_AMOUNT_INR * 100, // in paise (e.g. 100 = ₹1)
          currency: 'INR',
          name: 'Practical AI Masterclass',
          description: '1-Hour Live Masterclass Access Pass (₹1)',
          ...
          handler: function (_response: any) {
            ...
            try {
              ...
              if (existing.length > 0) {
                existing[existing.length - 1].paymentStatus = `Paid / Confirmed (₹1 via Razorpay - ${_response.razorpay_payment_id || 'Success'})`;
                localStorage.setItem('qnayds_masterclass_leads', JSON.stringify(existing));
              }
              ...
              if (GOOGLE_SHEET_WEBHOOK_URL) {
                const updatePayload = {
                  ...
                  occupation: occupation === 'Other' ? `Other: ${occupationOther}` : occupation,
                  source: 'AI Masterclass 1-Hour Live Pass (₹1)',
                  paymentStatus: `Paid (₹1 via Razorpay: ${_response.razorpay_payment_id || 'Success'})`
                };
    ...
    // Process and confirm payment, then automatically proceed to WhatsApp group step
    setTimeout(() => {
      try {
        const existing = JSON.parse(localStorage.getItem('qnayds_masterclass_leads') || '[]');
        if (existing.length > 0) {
          existing[existing.length - 1].paymentStatus = `Paid / Confirmed ($2 via ${method.toUpperCase()})`;
          localStorage.setItem('qnayds_masterclass_leads', JSON.stringify(existing));
        }
      } catch {
        // ignore
  ...
  const handleConfirmPaid = () => {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem('qnayds_masterclass_leads') || '[]');
      if (existing.length > 0) {
        existing[existing.length - 1].paymentStatus = 'Paid / Confirmed (₹1 via Razorpay)';
        localStorage.setItem('qnayds_masterclass_leads', JSON.stringify(existing));
      }
      // Send payment confirmation update to Google Sheet
      if (GOOGLE_SHEET_WEBHOOK_URL) {
        const updatePayload = {
          ...
          occupation: occupation === 'Other' ? `Other: ${occupationOther}` : occupation,
          source: 'AI Masterclass 1-Hour Live Pass (₹1)',
          paymentStatus: 'Paid (₹1 via Razorpay - Verified)'
        };
        fetch(GOOGLE_SHEET_WEBHOOK_URL, {
          method: 'POST',
          mode: 'no-cors',
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-slate-900 leading-tight">
                {currentStep === 'form' && 'Join 1-Hour Live Masterclass (₹1)'}
                {currentStep === 'payment' && 'Step 2: Razorpay Payment (₹1)'}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-blue-50/80 border border-blue-200 text-slate-800 text-xs sm:text-sm space-y-1">
                <div className="flex items-center gap-2 font-bold text-blue-900">
                  ...
                  <span>1-Hour Live Masterclass ($2) Registration</span>
                </div>
                <p className="text-slate-600 text-xs leading-relaxed">
                  Fill in your details below to register for the 1-Hour Live AI Masterclass. After submitting, you will be redirected straight to the official <strong className="text-blue-700">WhatsApp Cohort Group</strong>.
                </p>
              </div>
              ...
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 text-white p-4 sm:p-5 rounded-2xl shadow-xl relative overflow-hidden">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right bg-white/10 px-3.5 py-2.5 rounded-xl border border-white/15 min-w-[90px] shrink-0">
                    <span className="text-[9px] text-slate-400 block line-through">Regular ₹999</span>
              ...
              <div className="space-y-2.5 pt-1">
                ...
                >
                  {isPaymentVerifying ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
                      <span>Verifying ₹1 Payment & Unlocking WhatsApp...</span>
                    </>
                  ) : (
                    <>
                      <CreditCard className="w-5 h-5" />
                      <span>{paymentInitiated ? 'Re-open Razorpay Gateway (₹1)' : 'Pay ₹1 via Razorpay & UPI'}</span>
                      <ArrowRight className="w-5 h-5" />
                    </>
                  )}
                </button>
                ...
                {paymentInitiated && (
                  <div className="p-3 bg-emerald-50 border border-emerald-300 rounded-xl text-center space-y-2 animate-in fade-in">
                    <p className="text-xs font-semibold text-emerald-900 flex items-center justify-center gap-1.5">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                      <span>If you have completed the payment via UPI / Card, click below:</span>
                    </p>
                    <button
                      type="button"
                      onClick={handleConfirmPaid}
                      disabled={isPaymentVerifying}
                      className="w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm sm:text-base rounded-xl shadow-md shadow-emerald-600/30 flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >
                      <span>Continue & Join WhatsApp Group</span>
